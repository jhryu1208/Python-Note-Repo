...
"""
단순 복사 : 복사 대상과 완전히 동일하게 복사를 수행하는 것 (immutable한 객체에서는 발생 X)
얕은 복사 : 복사했다고 생각했지만 실제로는 변수가 동일 값을 참조하고 있는 것
깊은 복사 : 깊은 복사는 내부에 객체들까지 모두 새롭게 copy 되어 서로 다른 객체를 참조하고 있는 것
"""

r1 = ['string', (1, 2), [100, 200]]
r2 = r1 # r1이 참조하고 있는 리스트를 r2도 참조하게됨 (리스트의 레퍼런스 카운트 1증가) [단순복사에 해당]
r3 = list(r1) # r1이 참조하고 있는 리스트의 내용으로 새로운 리스트를 생성함 (즉, r2와 달리 참조하지 않음) [얕은복사에 해당]
...
(어짜피 수정하게되면 원본은 유지되고, 수정된 상태의 새로운 immutable객체를 생성하기 때문)
...
print(f'd1[2] == r1[2] : {d1[2] == r1[2]}') # 이전과 동일하게 값의 내용은 동일하지만
print(f'd1[2] is r1[2] : {d1[2] is r1[2]}') # 어떤 값을 참조하고 있는지는 서로 달라진 것을 알 수 있다.


"""
결론적으로,
얕은 복사와 깊은 복사에서 가장 중요한 것은
복사의 대상이 mutable객체인지 immutable객체인지 구분하는 것이다.
- immutable일 경우 크게 신경 쓸 필요 없지만,
- mutable일 때는 신경쓰는 것을 권장한다.
"""
